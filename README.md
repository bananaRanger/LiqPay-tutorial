...
В тестовом проекте я буду использовать `Swift`, но порядок действий не будет сильно отличаться для `Objective-C`, за исключением пункта ! для `Objective-C` его стоит пропустить.
